...
def get_user_function(**kwargs):
    ...
    product_identifier = user_function_context.get("product_identifier", None)
    ...
    if product_identifier is None:
        raise NotImplementedError(
            "To use a user-function to generate an user you need to pass"
            " in the `product_identifier` as entry in a dict argument called"
            " `context`"
        )
    ...
    if not module_fpath.exists():
        raise NotImplementedError(
            ...
            f" in {module_fpath.parent} containing a function called `{product_identifier}`"
    ...
    user_function = getattr(user_function_module, product_identifier, None)
    if user_function is None:
        raise NotImplementedError(
            f"Please define `{product_identifier}` in the user-function module source"
            f" file `{module_fpath}`"
        )
